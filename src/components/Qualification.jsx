    ...
    <section id="qualification" className="max-w-container-max mx-auto px-8 py-section-gap relative">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-primary-container/50 via-white/10 to-transparent"></div>
            ...
            >
              {currentData.map((item, index) => (
                <div key={index} className={`relative flex items-center w-full ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                  {/* Dot on line */}
                  <div className="absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-background border-2 border-primary-container z-10 shadow-[0_0_15px_rgba(75,142,255,0.5)]"></div>

                  {/* Content Box */}
                  <div className={`w-[42%] md:w-[45%] ${index % 2 === 0 ? 'text-right pr-4 md:pr-0' : 'text-left pl-4 md:pl-0'}`}>
                    <motion.div
                    ...
                    >
                      ...
                      <div className={`flex items-center gap-2 text-xs text-on-surface-variant opacity-60 mt-4 ${index % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
